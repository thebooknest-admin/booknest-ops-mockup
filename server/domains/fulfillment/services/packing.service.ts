...
export async function listPackingOrders() {
  ...
  const [membersRes, addressesRes] = await Promise.all([
    sbFetch(`/members?id=in.(${memberIds.join(",")})&select=id,name,tier,age_group&limit=200`),
  ...
  const orders = shipments.filter((s) => memberMap[s.member_id]).map((s) => {
    const m = memberMap[s.member_id];
    return { shipment_id: s.id, shipment_number: s.shipment_number, order_number: s.order_number, scheduled_ship_date: s.scheduled_ship_date, member_id: m.id, member_name: m.name, tier: m.tier, age_group: m.age_group, address: addressMap[m.id] ?? null };
  });
  return { orders };
}

export async function markShipmentPacked(input: { shipment_id: string }) {
  const [shipment] = await sbJson<{ id: string; status: string }[]>(
    `/shipments?id=eq.${input.shipment_id}&select=id,status&limit=1`
  );
  if (!shipment) throw new Error("Shipment not found.");
  if (shipment.status !== "packing") throw new Error(`Shipment is already ${shipment.status}; refresh the queue.`);
  const shipmentBooks = await sbJson<{ id: string; book_copy_id: string | null; status: string | null; book_copies: { status: string | null } | null }[]>(
    `/shipment_books?shipment_id=eq.${input.shipment_id}&select=id,book_copy_id,status,book_copies(status)&limit=200`
  );
  if (shipmentBooks.length === 0) throw new Error("Cannot pack a shipment with no books assigned.");
  if (shipmentBooks.find(book => !book.book_copy_id || book.status !== "picked")) {
    throw new Error("Cannot pack until every assigned book has been scanned in picking.");
  }
  if (shipmentBooks.find(book => !["in_house", "in_transit"].includes(book.book_copies?.status ?? ""))) {
  ...
  await sbVoid(`/shipments?id=eq.${input.shipment_id}`, {
    method: "PATCH",
    body: JSON.stringify({ status: "packed", updated_at: now }),
